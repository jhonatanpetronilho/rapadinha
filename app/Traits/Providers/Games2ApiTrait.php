   ...
   private static function ProcessPlayGames2($data, $userId, $type)
{
    ...
    if(!empty($wallet)) {
        ...
        // Verificar se é transação de vitória duplicada
        ...
        if(!empty($transaction)) {
            if(floatval($data[$type]['win']) > 0) {
                ...
                // Adicionar o ganho na tabela orders e atualizar status para 1
                Order::create([
                    'user_id'       => $wallet->user_id,
                    'session_id'    => $transaction->session_id,
                    'transaction_id'=> $data[$type]['txn_id'],
                    'type'          => 'win',
                    'type_money'    => $transaction->getAttributes()['type_money'],
                    'amount'        => $data[$type]['win'],
                    'providers'     => 'pgclone',
                    'game'          => $data[$type]['game_code'],
