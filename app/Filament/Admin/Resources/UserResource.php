...
use Filament\Tables\Actions\HeaderActionsPosition; // <--- Adicione esta linha
use Filament\Tables\Actions\Action; // <--- Adicione esta linha
...
class UserResource extends Resource
{
     ...
     */
    public static function sidebar(Model $table): FilamentPageSidebar // Mudei $record para $table para evitar conflito de nome no table()
    {
        return FilamentPageSidebar::make()
            ->setTitle($table->name)
            ->setDescription($table->created_at)
            ->setNavigationItems([
                ...
                PageNavigationItem::make(__('base.view_user'))
                    ->translateLabel()
                    ->url(static::getUrl('view', ['record' => $table->id]))->icon('heroicon-o-user')
                    ->isActiveWhen(function () {
                        return request()->routeIs(static::getRouteBaseName() . '.view');
                    }),
                PageNavigationItem::make(__('base.edit_user'))
                    ->translateLabel()
                    ->url(static::getUrl('edit', ['record' => $table->id]))
                ...
                PageNavigationItem::make(__('base.change_password'))
                    ->translateLabel()
                    ->url(static::getUrl('password.change', ['record' => $table->id]))
                    ->icon('heroicon-o-key')
                    ->isActiveWhen(function () {
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('wallet.total_balance')
                    ...
                    ->label('Saldo')
                    ->money('BRL'),
                Tables\Columns\TextColumn::make('vipLevel')
                    ->label('Level Vip'),
            ...
            ->headerActions([ // <--- Adicione este bloco aqui
                Action::make('downloadUsers')
                    ->label('Download de Usuários (CSV)')
                    ->icon('heroicon-o-arrow-down-tray')
                    ->color('success')
                    ->url(fn () => route('admin.users.download-csv')) // <--- Aponta para uma nova rota
                    ->openUrlInNewTab(), // <--- Abre o download em uma nova aba para evitar problemas
            ]);
    }
