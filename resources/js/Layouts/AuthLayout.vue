<template>
    ...
    <SideBarComponent v-if="showSidebar" v-once/>

    <div
        :class="mainContentClass" class="mt-[65px]">
...
export default {
    props: {
        showSidebar: {
            type: Boolean,
            default: true // Padrão é mostrar o sidebar
        }
    },
    components: {
        MissionWeekly,
    ...
    computed: {
        mainContentClass() {
            if (!this.showSidebar) {
                return ''; // Se não houver sidebar, não adiciona margem
            }
            // Se houver sidebar, usa a lógica original de expandir/encolher
            return this.sidebar === true ? 'md:ml-[65px]' : 'md:ml-[280px]';
        },
        isAuthenticated() {
